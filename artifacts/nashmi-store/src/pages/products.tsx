  ...
  const filteredProducts = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(
        (p) => p.name.includes(searchQuery) || p.description.includes(searchQuery) ||
  ...
  return (
    <div className="min-h-screen pt-24 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-10">
          <h1 className="text-4xl font-black text-white mb-2" style={{ fontFamily: "'Cairo', sans-serif", fontWeight: 900 }}>
            جميع المنتجات
          </h1>
          <p className="text-white/40">
            {loading ? "جارٍ التحميل..." : `${filteredProducts.length} منتج متاح`}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          ...
          <div className="relative">
            <SlidersHorizontal size={16} className="absolute top-1/2 -translate-y-1/2 right-4 text-white/30 pointer-events-none" />
          ...
          {Object.entries(categoryLabels).map(([key, label]) => (
            <button key={key} onClick={() => setActiveCategory(key)}
              className={`px-5 py-2 rounded-xl text-sm font-bold transition-all duration-200 ${
                activeCategory === key ? "text-white" : "text-white/50 border border-white/10 hover:border-white/20 hover:text-white/70"
              }`}
              style={activeCategory === key ? { background: "linear-gradient(135deg, #dc2626, #b91c1c)", boxShadow: "0 0 15px rgba(220,38,38,0.3)" } : {}}
        ...
        {loading && (
          <div className="flex flex-col items-center py-20 gap-4">
            ...
            <p className="text-white/40">جارٍ تحميل المنتجات...</p>
          </div>
        )}
        ...
        {!loading && error && (
          <div className="text-center py-16">
            ...
            <p className="text-white/30 text-sm mt-2">تأكد من اتصالك بالإنترنت</p>
          </div>
        )}
